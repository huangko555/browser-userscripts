...
// @compatible   edge 120+
// @noframes
// 注意：已移除原版的 @downloadURL / @updateURL，防止油猴自动更新覆盖掉本地修改。
// 原版地址：https://greasyfork.org/scripts/474507  ｜  源码：https://github.com/posthumz/wider-bilibili
// ==/UserScript==
...
/* 原宽屏按钮不显示（hkq fix：原本这里还隐藏了 .bpx-player-ctrl-web 网页全屏按钮，已移除该行以恢复按钮；该按钮的点击行为由下方 JS 接管，用来切换"预留高度归零") */
...
::-webkit-scrollbar{width:0!important;height:0!important}`);
  ...
  switch (url.host) {
    case "www.bilibili.com": {
      ...
      // hkq fix：番剧页(Next.js)里播放器被脚本绝对定位拉到顶部后，左列容器 .plp-left-wrap 仍用
      // padding-top(约276px)给播放器预留位置，与脚本顶部预留双重叠加 → 播放器下方大片空白。
      // 收掉这段 padding-top 即可（顺带把播放器占位 .plp-player 高度也收掉）。
      // .plp-* 是番剧页独有类，普通视频页无此元素，故全局加无副作用。
      GM_addStyle(`.plp-left-wrap{padding-top:0!important}
.plp-player{height:0!important;min-height:0!important}`);
      await( waitReady());
  ...
  bottom: ${bt}px !important;
      ...
      // ===== hkq fix：用「网页全屏」按钮模拟网页全屏 =====
      // 原生网页全屏被脚本锁死（强制 data-screen=web），点了没用、只会让内容靠左。
      // 这里把按钮的点击行为接管：点一下 = 把"预留高度"临时清空（播放器顶满视口 ≈ 网页全屏），
      // 再点一下 = 恢复成你在设置面板里设定的预留高度；切换到别的视频会自动恢复为设定值。
      GM_addStyle(`.bpx-player-ctrl-web.hkq-webfull-on{color:#00aeec!important}`);
      let hkqWebFull = false;
      let hkqSavedReserve = null;
      const hkqApplyWebFull = () => {
        const root = document.documentElement;
        if (hkqWebFull) {
          // 记下用户当前设定的预留高度，再清空（等价于把"预留高度"选项设为 0）
          hkqSavedReserve = root.style.getPropertyValue("--reserve-height-input");
          root.style.removeProperty("--reserve-height-input");
        } else {
          // 恢复用户设定值
          if (hkqSavedReserve) root.style.setProperty("--reserve-height-input", hkqSavedReserve);
          else root.style.removeProperty("--reserve-height-input");
          hkqSavedReserve = null;
        }
        for (const b of document.getElementsByClassName("bpx-player-ctrl-web"))
          b.classList.toggle("hkq-webfull-on", hkqWebFull);
      };
      // 捕获阶段拦截，抢在 B站自己的处理之前接管点击
      document.addEventListener("click", (ev) => {
        const t = ev.target;
        if (!(t instanceof Element) || !t.closest(".bpx-player-ctrl-web")) return;
        ev.stopImmediatePropagation();
        ev.preventDefault();
        hkqWebFull = !hkqWebFull;
        hkqApplyWebFull();
        // 进入网页全屏时把页面瞬间置顶（behavior:instant 强制无动画，覆盖页面的平滑滚动设置）
        if (hkqWebFull) window.scrollTo({ top: 0, left: 0, behavior: "instant" });
      }, true);
      // 切换视频（路径变化）时恢复为用户设定的预留高度
      let hkqLastPath = location.pathname;
      const hkqOnNav = () => {
        if (location.pathname === hkqLastPath) return;
        hkqLastPath = location.pathname;
        if (hkqWebFull) { hkqWebFull = false; hkqApplyWebFull(); }
      };
      ...
      window.addEventListener("popstate", hkqOnNav);
      // ===== hkq fix 结束 =====
